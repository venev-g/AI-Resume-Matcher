    ...
    async def call_llm(
        ...
        **kwargs
        ...
        if provider == "gemini" and self.gemini_client:
            return await self._call_gemini(prompt, max_retries, delay, temperature, max_tokens, **kwargs)
        elif provider == "openrouter" and settings.OPENROUTER_API_KEY:
            return await self._call_openrouter(prompt, max_retries, delay, temperature, max_tokens, **kwargs)
        else:
        ...
        **kwargs
        ...
        payload = {
            ...
            "messages": [
                {
                    "role": "system",
                    "content": kwargs.get('system_message', "You are a helpful AI assistant specialized in resume and job description analysis.")
                },
                {
                    "role": "user",
                    "content": prompt
        ...
        for attempt in range(max_retries):
            try:
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.post(
                        ...
                        if response.status == 200:
                            ...
                            raise LLMServiceError("Empty or invalid response from OpenRouter API")
                            ...
                            if response.status == 429:  # Rate limit
                                ...
                                raise LLMServiceError(f"OpenRouter API error {response.status}: {error_text}")
                ...
                logger.error(f"OpenRouter API client error on attempt {attempt + 1}: {e}")
                if attempt == max_retries - 1:
                    raise LLMServiceError(f"OpenRouter API client error: {str(e)}")
            except Exception as e:
                logger.error(f"OpenRouter API error on attempt {attempt + 1}: {type(e).__name__} - {e}")
                if attempt == max_retries - 1:
                    raise LLMServiceError(f"All OpenRouter API retry attempts failed: {str(e)}")
