...
async def extract_resume_skills_node(state: ResumeMatchingState) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "extracted_resume_skills_json": result,
            "processing_status": "resume_skills_extracted"
        }
...
async def validate_resume_skills_node(state: ResumeMatchingState) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "validated_resume_skills_json": validated_skills,
            "processing_status": "resume_skills_validated"
        }
...
async def classify_resume_skills_node(state: ResumeMatchingState) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "classified_resume_skills_json": result,
            "processing_status": "resume_skills_classified"
        }
...
async def extract_jd_skills_node(state: ResumeMatchingState) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "extracted_jd_skills_json": result,
            "processing_status": "jd_skills_extracted"
        }
...
async def validate_jd_skills_node(state: ResumeMatchingState) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "validated_jd_skills_json": validated_skills,
            "processing_status": "jd_skills_validated"
        }
...
async def classify_jd_skills_node(state: ResumeMatchingState) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "classified_jd_skills_json": result,
            "processing_status": "jd_skills_classified"
        }
...
async def match_skills_node(state: ResumeMatchingState) -> Dict[str, Any]:
    ...
    try:
        logger.info(f"Matching skills for resume {state['resume_id']} against JD {state['jd_id']}")
        ...
        try:
            ...
            matched_skills = [item["skill"] for item in skill_analysis.get("perfect_matches", [])]
            matched_skills.extend([item["skill"] for item in skill_analysis.get("good_matches", [])])
            
            missing_skills = [item["skill"] for item in skill_analysis.get("missing_critical", [])]
            missing_skills.extend([item["skill"] for item in skill_analysis.get("missing_preferred", [])])
            
            additional_skills = [item["skill"] for item in skill_analysis.get("bonus_skills", [])]
...
async def generate_suggestions_node(state: ResumeMatchingState) -> Dict[str, Any]:
    ...
    try:
        ...
        if match_percentage >= 80.0:
            logger.info(f"Match percentage {match_percentage}% is above threshold. No suggestions needed.")
            return {
